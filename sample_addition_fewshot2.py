"""
This is for on-the-fly version (data_type = 'text')
"""
import os
import pickle
...
def get_num_prompt_and_set(x):
    ...
    x = x.split('/')[-1]
    num_prompt = int(''.join(x for x in x.split('_')[0] if x.isdigit()))
    num_set = int(''.join(x for x in x.split('_')[1] if x.isdigit()))
...
for ckpt_path_name in ckpt_list:
    ...
    if multiple_set_per_prompt:
        ...
        # start_pattern = '_'.join(prompt_dir.split('_')[:-2])
        # start_list_all = glob.glob(start_pattern+'_*_*.txt')
        start_pattern = '/'.join(prompt_dir.split('/')[:-1])
        start_list_all = glob.glob(start_pattern+'/*_*.txt')
        ...
        start_pattern = '_'.join(prompt_dir.split('_')[:-1])
