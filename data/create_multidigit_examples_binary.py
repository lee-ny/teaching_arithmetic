...
# python create_multidigit_examples_binary.py --total_num_examples=10000 --num_digit=10 --ver=1
# python create_multidigit_examples_binary.py --total_num_examples=10000 --num_digit=10 --ver=2
...
if ver == 1:
    ...
    if num_per_digit > 256: 
        num_per_digit = (total_num_examples-256) // (num_digits-4)
        p_list = [4,12,48,192] + [num_per_digit for i in range(5, num_digits+1)]
        p_list[4] += total_num_examples - sum(p_list) # give the remaining examples to p_2
    else:
        print('disreagarding this case for now') # TODO:
    ...
    p = (total_num_examples - 256)  / (2**(num_digits-4)-1)
    p_list = [4,12,48,192] + [int(2 ** (i-5)*p) for i in range(5, num_digits+1)]
    p_list[4] += total_num_examples - sum(p_list) # give the remaining examples to p_2

print('number of examples for each digit: ', p_list)
p_list_percentage = [100* p_list[i]/(2**(2*(i+1))) for i in range(len(p_list))]



# if folder 'addition_multidigit' does not exist, create it
out_dir = f'addition_multidigit_bal_binary_ver{ver}'
...
if os.path.exists(output_filename):
    ...
    with open(output_filename, 'w') as f:
        ...
        for a in range(16): # handle the 1~4 digit addition cases
            for b in range(16):
                c = a + b
                num_carry = numCarryOps(a, b)
                num_carry_list[num_carry] += 1
                bin_a, bin_b, bin_c = convert_to_binary(a), convert_to_binary(b), convert_to_binary(c)
                c_rev = reverse_string(str(bin_c))
                # write the example to file
                f.write(f'${bin_a}+{bin_b}={c_rev}$\n')

        num_example = 0
        for num_digit in range(5, num_digits+1):
            num_digit_example = 0
